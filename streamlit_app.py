    ...
    if st.session_state.trigger_execution:
        ...
        if not user_prompt:
            ...
            with st.spinner("🧠 Processing your prompt..."):
                ...
                try:
                    # --- NEW: Classify intent using LiquidEngine ---
                    intent = liquid_engine.process_prompt(user_prompt)
                    print(f"[LiquidEngine] Detected intent: {intent}")
                    # --- END NEW ---
    ...
    if not logs:
        ...
        for log in reversed(filtered_logs):
            with st.expander(f"📅 {log['timestamp']} | Route: {log['route']} | Status: {log['status']}"):
                ...
                if "intent" in log:  # --- NEW: Show intent in logs ---
                    st.markdown(f"**Intent:** {log['intent']}")
                if "dag" in log:
                    st.json(log["dag"])
                if "results" in log:
                    st.json(log["results"])
