...
- Fill out the Backend/orchestration/agent runtime modules (`Backend/orchestration/`, `Backend/agents/`).
- REST/WebSocket API hardening and schema coverage (`Backend/APIs/`).
...
- Multimodal capability-gated pipeline (`Backend/multimodal/`).
...
- **Framework 2** — `Backend/unified_orchestrator/`: hierarchical parallel DAG orchestrator
  with NLTK sentence parsing, coreference mapping, and Graphviz DAG visualization.
- **Framework 3** — `Backend/smart_orchestrator/`: template-based Bartender decomposition
  with live task metrics and telemetry routing.
- **Framework 4** — `Backend/revolutionary_orchestrator/`: fully offline LLMCompiler-style
...
- Intent classifier fine-tuning pipeline over `typeform/distilbert-base-uncased-mnli`
  via `Research/training/train_classifier.py` and `Research/training/domain_dataset.json`.
