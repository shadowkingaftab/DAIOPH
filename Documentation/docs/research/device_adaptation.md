...
### 1. Model Selection (`Backend/hardware/model_scaler.py`)
...
### 2. Quantization Strategy (`Backend/models/optimization/quantization.py`)
...
Implemented via `Backend/intelligence/liquid/adaptation.py` adjusting strategy parameters.

### 4. Batching & Concurrency (`Backend/models/optimization/batching.py`)

- Small devices: batch size 1, serialized requests
- Servers: dynamic batching, parallel inference streams

### 5. Energy-Aware Scheduling (`Backend/hardware/energy_manager.py`)
...
`Backend/learning/adaptation/device_adaptation.py` learns:
...
Device benchmarks in `Research/benchmarks/devices/` establish baselines:
...
From `Research/evaluation/Backend/models/efficiency.py`:
