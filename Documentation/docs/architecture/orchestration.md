...
|--------------|----------|----------|
| Hybrid | `Backend/core/hybrid_orchestrator.py` | Default: local + remote mix |
| Revolutionary | `Backend/revolutionary_orchestrator/` | Experimental strategies |
| Smart | `Backend/smart_orchestrator/` | Heuristic-driven routing |
| Unified | `Backend/unified_orchestrator/` | Single entry point facade |

## Execution Pipeline (`Backend/execution/`)

### DAG Generator (`dag_generator.py`)
Converts plans into directed acyclic graphs:
- Nodes = tasks with dependencies
- Edges = Research/data/control flow
...
## Agent Coordination (`Backend/agents/`)
...
## Routing (`Research/evaluation/Backend/orchestration/routing.py`)
...
## Recovery (`Research/evaluation/Backend/orchestration/recovery.py`)
