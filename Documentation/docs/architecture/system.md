...
|-----------|----------|----------------|
| Kernel | `Backend/core/kernel/` | Boot, runtime, event loop, lifecycle |
| Contracts | `Backend/core/contracts/` | Protocols, commands, events, messages |
| Identity | `Backend/core/identity/` | Device, user, session identity |
| Configuration | `Backend/core/configuration/` | Config loading, profiles, feature flags |
| Intelligence | `Backend/intelligence/` | Liquid engine, intent, reasoning, cognition |
| Memory | `Backend/memory/` | Short-term, episodic, semantic, procedural |
| Learning | `Backend/learning/` | Continual learning, adaptation, evolution |
| Models | `Backend/models/` | Registry, local providers, remote providers |
| Multi-modal | `Backend/multi_modal/` | Input routing, speech, vision, video |
| Agents | `Backend/agents/` | Planner, executor, monitor, verifier roles |
| Hardware | `Backend/hardware/` | Detection, CPU/GPU, energy management |
| OS Layer | `Backend/os_layer/` | Desktop, filesystem, automation |
| Federation | `Backend/federated/` | Client/server FL with privacy |
| Network | `Backend/network/` | Discovery, transport, synchronization |
