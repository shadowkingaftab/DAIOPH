...
DAIOPH exposes a REST API via FastAPI (`Backend/APIs/rest/app.py`). The default endpoint is `http://localhost:8000`.
...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/Backend/memory/episodes` | List episodic memories |
| GET | `/api/Backend/memory/search?q=` | Semantic search |
| DELETE | `/api/Backend/memory/{id}` | Delete a memory |
...
| POST | `/api/Backend/models/load` | Load a model |
| POST | `/api/Backend/models/unload` | Unload a model |
...
Error codes align with `Backend/core/errors/`.
