...
DAIOPH uses an internal event bus (`Backend/APIs/events/event_bus.py`) for decoupled communication between subsystems. Events are typed, ordered, and optionally persisted.

## Event Structure

Defined in `Backend/core/contracts/events.py`:
...
- State reconstruction via replay (`Backend/intelligence/state/state_recovery.py`)
- Audit trails
