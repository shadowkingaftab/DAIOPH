...
```bash
cd Operations/deployment/docker
docker compose up -d
```
...
Using raw manifests:
```bash
kubectl apply -f Operations/deployment/kubernetes/namespace.yaml
kubectl apply -f Operations/deployment/kubernetes/
```

Or Helm (recommended):
```bash
helm install daioph Operations/deployment/helm \
...
```bash
cd Operations/deployment/terraform
terraform init
...
Production settings via environment variables or `Operations/configs/production/config.yaml`:
