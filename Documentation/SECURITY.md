...
- Authentication, authorization, and session handling (`Backend/security/`).
- Sandbox escapes or unsafe filesystem/process tool execution
  (`Backend/tools/filesystem/`, `Backend/tools/system/`, `Backend/os_layer/`).
- Prompt-injection and threat-detection bypasses (`Backend/security/threat_detection/`).
- Secret leakage via logs, telemetry, exports, or Docker images.
- Insecure defaults in API endpoints (`Backend/APIs/rest/`, `Backend/APIs/websocket/`).
...
- **Sandboxing**: filesystem and process policies under `Backend/security/sandbox/`
  constrain what tool executions may touch.
- **Audit trail**: security-relevant events emit structured audit records
  (`Backend/security/audit/`) with correlation IDs; user content is redacted before
  export (`Backend/observability/telemetry/privacy_filter.py`).
