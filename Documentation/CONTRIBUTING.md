...
- **Error handling**: use the typed hierarchy rooted at `Backend/core/errors/base.py`;
...
Tests live under `Quality/tests/` (unit, integration, system, end-to-end, security,
resilience). pytest is configured with `testpaths = ["tests"]`.

```bash
pytest                       # full suite
pytest Quality/tests/unit -q         # fast subset during development
pytest Quality/tests/unit/test_memory.py -q
```
