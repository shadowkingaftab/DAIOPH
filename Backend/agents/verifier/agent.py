...
:class:`VerifierAgent` extends :class:`agents.base.agent.BaseAgent`. Behaviour:
...
from agents.verifier.prompts import ROLE, build_prompt

__all__ = ["VerifierAgent", "AgentCapabilityError"]
...
class VerifierAgent(BaseAgent):
    """Agent responsible for: Check an artifact against explicit acceptance criteria and emit a pass/fail verdict with per-criterion detail."""
    ...
    def _local_fallback(self, task: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        artifact_text = str(task)
        checks = {}
        for criterion in criteria:
            checks[criterion] = criterion.lower() in artifact_text.lower()
        if not criteria:
            checks["non_empty"] = bool(artifact_text.strip())
        return {
