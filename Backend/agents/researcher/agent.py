...
:class:`ResearcherAgent` extends :class:`agents.base.agent.BaseAgent`. Behaviour:
...
from agents.researcher.prompts import ROLE, build_prompt

__all__ = ["ResearcherAgent", "AgentCapabilityError"]
...
class ResearcherAgent(BaseAgent):
    """Agent responsible for: Gather and cite sources for a question using the injected search callable."""
    ...
    def _local_fallback(self, task: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not callable(search):
            raise AgentCapabilityError(
                "ResearcherAgent needs an injected LLM or a callable in "
                "context['search']; no network access is performed implicitly"
            )
        return {"findings": search(task, context), "source": "search_callable"}
