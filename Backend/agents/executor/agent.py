...
:class:`ExecutorAgent` extends :class:`agents.base.agent.BaseAgent`. Behaviour:
...
from agents.executor.prompts import ROLE, build_prompt

__all__ = ["ExecutorAgent", "AgentCapabilityError"]
...
class ExecutorAgent(BaseAgent):
    """Agent responsible for: Carry out one assigned step by invoking the handler supplied in the run context."""
    ...
    def _local_fallback(self, task: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        handler = context.get("execute")
        if not callable(handler):
            raise AgentCapabilityError(
                "ExecutorAgent needs either an injected LLM or a callable "
                "in context['execute']; refusing to fake execution"
            )
        return {"output": handler(task, context), "source": "handler"}
