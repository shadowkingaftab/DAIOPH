...
:class:`PlannerAgent` extends :class:`agents.base.agent.BaseAgent`. Behaviour:
...
from agents.planner.prompts import ROLE, build_prompt

__all__ = ["PlannerAgent", "AgentCapabilityError"]
...
class PlannerAgent(BaseAgent):
    """Agent responsible for: Decompose a goal into an ordered list of concrete, executable steps with explicit dependencies."""
    ...
    def _local_fallback(self, task: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        parts = [p.strip() for p in _split_steps(task)]
        steps = []
        previous = None
        for i, part in enumerate(parts):
            step_id = f"step_{i}"
            entry = {"id": step_id, "description": part,
                     "depends_on": [previous] if previous else []}
            steps.append(entry)
            previous = step_id
        return {"plan": {"steps": steps}, "source": "local_heuristic"}
